...
namespace mysplit
{
    ...
    auto split(Iter begin, Iter end, const std::basic_regex < std::remove_const_t < std::remove_reference_t<decltype(*Iter{}) >> > & r = get_ws_regex < std::remove_const_t < std::remove_reference_t<decltype(*Iter{}) >> > ())
    {
        ...
        auto iter_begin = std::regex_token_iterator(&*begin, &*end, r, -1), iter_end = decltype(iter_begin){};

        if constexpr (support_emplace_back)
        {
            for (; iter_begin != iter_end; ++iter_begin)
                if (iter_begin->second != iter_begin->first)
                    container.emplace_back(&*iter_begin->first, iter_begin->second- iter_begin->first);
        }
        else if constexpr (support_emplace)
        {
            for (; iter_begin != iter_end; ++iter_begin)
                if (iter_begin->second != iter_begin->first)
                    container.emplace(&*iter_begin->first, iter_begin->second- iter_begin->first);
        }
        else if constexpr (support_emplace_after)
        {
            ...
            for (; iter_begin != iter_end; ++iter_begin)
                if (iter_begin->second != iter_begin->first)
                    container_iter = container.emplace_after(container_iter, &*iter_begin->first, iter_begin->second- iter_begin->first);
        }
        else
    ...
    void split_to(Iter begin, Iter end, inserter_type&& ins, const std::basic_regex < std::remove_const_t < std::remove_reference_t<decltype(*Iter{}) >> > & r = get_ws_regex < std::remove_const_t < std::remove_reference_t<decltype(*Iter{}) >> > ())
    {
        for (auto iter = std::regex_token_iterator(begin, end, r, -1), enditer = decltype(iter){}; iter != enditer; ++iter)
            if (iter->second > iter->first)
                ins = { &*iter->first,static_cast<size_t>(iter->second - iter->first) };
    }
